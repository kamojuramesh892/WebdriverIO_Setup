...
export async function getBase64FullPageScreenshotsData(takeScreenshot, executor, options) {
    const { addressBarShadowPadding, devicePixelRatio, deviceRectangles, fullPageScrollTimeout, hideAfterFirstScroll, innerHeight, isAndroid, isAndroidNativeWebScreenshot, isAndroidChromeDriverScreenshot, isIOS, screenHeight, screenWidth, toolBarShadowPadding, } = options;
 ...
 */
export async function getMobileFullPageNativeWebScreenshotsData(takeScreenshot, executor, options) {
    ...
    const { addressBarShadowPadding, devicePixelRatio, deviceRectangles: { viewport }, fullPageScrollTimeout, hideAfterFirstScroll, isAndroid, isLandscape, toolBarShadowPadding, } = options;
    ...
    for (let i = 0; i <= amountOfScrollsArray.length; i++) {
        ...
        const imageYPosition = (amountOfScrollsArray.length === i ? viewportHeight - imageHeight : 0) + viewportY + addressBarShadowPadding;
 ...
 */
export async function getAndroidChromeDriverFullPageScreenshotsData(takeScreenshot, executor, options) {
    const viewportScreenshots = [];
 ...
 */
export async function getDesktopFullPageScreenshotsData(takeScreenshot, executor, options) {
    const viewportScreenshots = [];
    const { devicePixelRatio, fullPageScrollTimeout, hideAfterFirstScroll, innerHeight } = options;
 ...
 * @TODO: remove the any
 */
function logHiddenRemovedError(error) {
 ...
 */
export async function takeWebElementScreenshot({ devicePixelRatio, deviceRectangles, element, executor, fallback = false, innerHeight, isAndroidNativeWebScreenshot, isAndroid, isIOS, isLandscape, screenShot, takeElementScreenshot, }) {
    if (fallback) {
    ...
    try {
        const base64Image = await takeElementScreenshot((await element).elementId);
