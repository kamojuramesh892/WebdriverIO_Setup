 ...
 */
export async function getMobileScreenSize({ currentBrowser, executor, isIOS, isNativeContext, }) {
    let height = 0, width = 0;
    const isLandscapeByOrientation = (await currentBrowser.getOrientation()).toUpperCase() === 'LANDSCAPE';
    try {
        if (isIOS) {
            ({ screenSize: { height, width } } = (await executor('mobile: deviceScreenInfo')));
            // It's Android
        }
        else {
            const { realDisplaySize } = (await executor('mobile: deviceInfo'));
 ...
 */
export async function loadBase64Html({ executor, isIOS, url }) {
    const htmlContent = `
        <html>
        <head>
 ...
 */
export async function executeNativeClick({ executor, isIOS, x, y }) {
    if (isIOS) {
 ...
 */
export async function getMobileViewPortPosition({ initialDeviceRectangles, isAndroid, isIOS, isNativeContext, methods: { executor, getUrl, url, }, nativeWebScreenshot, screenHeight, screenWidth, }) {
 ...
 */
export function canUseBidiScreenshot(methods) {
